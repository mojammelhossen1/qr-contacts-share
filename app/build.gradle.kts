...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.contactqr.ckqzpm"
    minSdk = 24
    targetSdk = 36
    versionCode = 1
    versionName = "1.0"
  ...
  signingConfigs {
    create("release") {
      ...
      if (customKeystorePath != null && file(customKeystorePath).exists()) {
        ...
        // Fallback to local debug keystore so release builds can be assembled locally without error
...
dependencies {
  ...
  // Lightweight ZXing Core library for QR matrix generation (<3MB app target)
